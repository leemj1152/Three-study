import { Box, OrbitControls, Sphere, useFaceControls, useKeyboardControls } from "@react-three/drei";
...
export const Experience = () => {
  ...
  const handleMovement = () => {
    ...
    if (rightPressed) {
      cube.current.applyImpulse({x: 0.2, y: 0, z: 0});
    }
    if (leftPressed) {
      cube.current.applyImpulse({x: -0.2, y: 0, z: 0});
    }
    if (forwardPressed) {
      cube.current.applyImpulse({x: 0, y: 0, z: -0.2});
    }
    if (backPressed) {
      cube.current.applyImpulse({x: 0, y: 0, z: 0.2});
    }
  ...
  return (
    ...
    onCollisionExit={({other}) => {
      ...
      <Box onPointerEnter={() => setHover(true)} onPointerLeave={() => setHover(false)} onClick={() => setStart(true)}>
